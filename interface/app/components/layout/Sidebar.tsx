      ...
      {isOpen ? (
        <div className="w-full flex items-center justify-between space-x-3 px-3 h-10 py-0 rounded-lg bg-gray-900">
          <div className="flex items-center space-x-3 flex-1">
            <ToastIcon />
            <span className="text-white font-semibold">VibeJam</span>
          </div>
...
function getActiveTabFromPathname(pathname: string, searchParams?: URLSearchParams | null): string {
  ...
  // Check if we're on the tutorial (task=tutorial query param on /vibe)
  if (pathname === '/vibe' && searchParams && searchParams.get('task') === 'tutorial') {
    return 'tutorial';
  }
...
export default function Sidebar({ 
  ...
  const navItemsRaw = [
    ...
    { id: 'tutorial', icon: FlaskConical, label: 'Tutorial', tooltip: 'Tutorial' },
    { id: 'about', icon: Info, label: 'Instructions' },
  ];
  const navigationItems = navItemsRaw as readonly { id: string; icon: typeof Grid3X3; label: string; tooltip?: string; isExternal?: boolean; externalUrl?: string }[];

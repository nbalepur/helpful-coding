...
import {
  ...
  createDefaultScores,
} from "@/app/constants/submissionRatingCriteria";
      ...
      try {
        ...
        RATING_DIMENSIONS.forEach((dimension) => {
          ...
          if (Number.isFinite(numeric)) {
            nextScores[dimension.key] = Math.min(
              RATING_SCALE_MAX,
              Math.max(RATING_SCALE_MIN, Math.round(numeric))
            );
          }
        });
      ...
      .map((submission) => {
        ...
        const averageScore =
          typeof ratingSummary?.average === "number" && Number.isFinite(ratingSummary.average)
            ? Math.max(RATING_SCALE_MIN, Math.min(RATING_SCALE_MAX, ratingSummary.average))
            : null;
        const totalRatings = ratingSummary?.count ?? 0;
        const ratingStarsHtml = (() => {
          const effectiveAverage = averageScore ?? 0;
          let html = "";
          for (let i = RATING_SCALE_MIN; i <= RATING_SCALE_MAX; i += 1) {
        ...
        const ratingAriaLabel =
          totalRatings > 0 && averageScore !== null
            ? `Rated ${ratingValueDisplay} out of ${RATING_SCALE_MAX} (${totalRatings} rating${totalRatings === 1 ? "" : "s"})`
            : "No ratings yet";
          const footerHtml =
                        ...
                        {RATING_DIMENSIONS.map((dimension) => (
                          <div key={dimension.key} className="flex flex-col gap-1">
                            <label className="flex items-center gap-2 text-sm font-medium text-gray-300">
                              <span>{dimension.name}: <span className="font-light">{ratingScores[dimension.key] ?? RATING_SCALE_DEFAULT}</span></span>
                              <span
                            ...
                            <input
                              ...
                              step={1}
                              value={ratingScores[dimension.key] ?? RATING_SCALE_DEFAULT}

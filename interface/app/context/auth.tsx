...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    // Always validate with backend when we have cookies (e.g. user may have been deleted).
    if (parsedUser && storedToken) {
      setUser(parsedUser);
    ...
    fetch(`${ENV.BACKEND_URL}/auth/validate`, {
      ...
      .then(async (response) => {
        ...
        const normalizedUser: User = {
          ...
          can_view_submissions: !!data.user.can_view_submissions,
        };
  ...
  const login = (userData: User, authToken: string) => {
    const normalizedUser: User = {
      ...
      can_view_submissions: !!userData.can_view_submissions,
    };
